...
parser.add_argument("--gpu", type=str, help="GPU index", default="0") #this
...
args.fs = 8000
args.fps = 25.0
args.offset = 10#changed
args.increment = args.fs/args.fps
args.cuda = torch.cuda.is_available() #this
...
with torch.no_grad():
    ...
    # Set emotion label to neutral (label 4)
    emo_label = torch.from_numpy(to_categorical(4, num_classes=6)).float().unsqueeze(0)
    print(emo_label)
    ...
    utils.write_video_cv(frame_list[:-args.offset, ...], speech[0, 0, :int(-args.offset*args.increment)], sr, output_path, 'neutral_generated.mp4', args.fps)
